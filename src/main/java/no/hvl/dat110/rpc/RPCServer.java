...
public class RPCServer {
	...
	public void run() throws IOException {
		...
		while (!stop) {
			...
			byte[] requestPayload = RPCUtils.decapsulate(requestBytes);

			rpcid = requestBytes[0];
			RPCRemoteImpl method = services.get(rpcid);

			byte[] returnValue = method.invoke(requestPayload);
			byte[] returnValueDecapsulated = RPCUtils.encapsulate(rpcid, returnValue);
			
			connection.send(new Message(returnValueDecapsulated)); 
			// TODO - END
